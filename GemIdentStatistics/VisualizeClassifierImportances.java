...
public class VisualizeClassifierImportances {
	 ...
	 * @param datumSetup 
	 */
	public VisualizeClassifierImportances(int[] importances, double error, DatumSetup datumSetup) {
		if (!(datumSetup instanceof DatumSetupThatUsesRings)){
			return;
		}
		maximum_radius = ((DatumSetupThatUsesRings)datumSetup).getMaximumRadius();
		
		this.error = ThreeDecimalDigitFormat.format(error * 100) + "%";
		LinkedHashMap<String, Color> filterNamesToColors = datumSetup.getFilterNamesToColors();
	...
	private void AddTickMarks(int c, Graphics g) {
		...
		for (int r = 0; r <= maximum_radius; r++){
			g.drawString(String.valueOf(r), (int)Math.round(io + delta * r), jo);
		}
	}
	...
	private void CreateBars(int c, String channel, int representation_color) {
		...
		for (int r = 0; r <= maximum_radius; r++){
//			System.out.println("create bar: " + channel + " r: " + r + " d:" + scaled_importances[c * (Datum.R + 1) + r]);
