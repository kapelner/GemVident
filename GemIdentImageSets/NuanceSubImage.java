...
public class NuanceSubImage extends DataImage {
	...
	protected HashMap<String, ShortMatrix> nuanceimages;
	protected int num;
...
//		System.out.println("setupDisplayImage w:" + example.getWidth() + " h:" + example.getHeight());
	...
	public void BuildDisplayImageFromWaves() {
		...
		for (String wave_name : imagelist.getWaveNames()){
			if (imagelist.getWaveVisible(wave_name)){
				...
				ShortMatrix pixels = nuanceimages.get(wave_name);
				Color displaycolor = imagelist.getWaveColor(wave_name);
				
				for (int i=0;i<displayimage.getWidth();i++){
					for (int j=0;j<displayimage.getHeight();j++){
						short val = pixels.get(i, j);
	...
	//					System.out.println("percent: " + percent + " i: " + i + " j: " + j + " cumu: " + cumulativecolorvaluematrix[i][j][0] + " " + cumulativecolorvaluematrix[i][j][1] + " " + cumulativecolorvaluematrix[i][j][2] + " " + cumulativecolorvaluematrix[i][j][3]);					
					}
				}
	...
	public static final double conversion_factor = (HistogramWidth + 1) / (double)(40001); //David Fletcher Holmes says so
...
//			System.out.println("final values: " + a + " " + b);
		...
		private void BinPixelValues() {
			ShortMatrix pixel_data = nuanceimages.get(wave_name);
			for (int i=0;i<displayimage.getWidth();i++){
				for (int j=0;j<displayimage.getHeight();j++){
//					System.out.print(pixel_data[i][j] + ", ");
...
//			for (int b : bins)
//				showbins += b + ", ";
//			System.out.println("showbins: " + showbins);			
		}
...
//				System.out.println("height: " + height + " of 150, max: " + bin_max_value);
...
//		System.out.println("CreateAdjustColorDialog");
...
//					    	System.out.println("visible_check deselected");
					    	imagelist.setWaveVisible(wave_name, false);
					    }
					    else {
//					    	System.out.println("visible_check selected");
...
//		System.out.println("getWaveImages: " + filename + " built:" + this.already_built);
		return wave_images.values();
	}
	
	public short getIntensityAtPoint(int i, int j, int wavenum){
		return nuanceimages.get(imagelist.getWaveName(wavenum)).get(i, j);
	}
	
	public HashMap<String, ShortMatrix> getPixelData() { //basically - get scores
		return nuanceimages;
	}
...
//		System.out.println("getAsBufferedImage in NuanceSubImage");
		if (nuanceimagepicpanel == null)
