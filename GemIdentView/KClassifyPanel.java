...
//	private Integer nRand;
	...
	private JRadioButton segmentation_method_radio_button;

	private JButton centroidSettingsButton;

	private JFrame settingsFrame;

	private Box centroidSettingsBox;

	private JCheckBox multipleFramesCheck;
...
//	}
	...
	private void SetUpOptions() {
		...
		Box multipleFramesBox=Box.createHorizontalBox();		
		JLabel multipleFramesLabel = new JLabel("Use Multiple Frames");
		multipleFramesLabel.setFont(FontForLabels);
		multipleFramesBox.add(multipleFramesLabel);
		multipleFramesCheck = new JCheckBox();
		multipleFramesCheck.setSelected(Run.DEFAULT_MULTIPLE_FRAMES_CHECK==1);
		multipleFramesBox.add(multipleFramesCheck);
		optionBox.add(multipleFramesBox);
		
		optionBox.add(Box.createVerticalStrut(10));
		
//		rotationalLabel=new JLabel("<html><u>Rotational Invariance Parameters</u></html>");
...
//	    classifyGroup.add(thoseClickedOnButton);
	    ...
	    Box allButtonBox = Box.createHorizontalBox();
	    allButtonBox.add(allButton);
	    optionBox.add(allButtonBox);
	    Box onlyButtonBox = Box.createHorizontalBox();
	    onlyButtonBox.add(onlyButton);
	    optionBox.add(onlyButtonBox);
	    Box remainButtonbox = Box.createHorizontalBox();
	    remainButtonbox.add(remainButton);
//	    optionBox.add(remainButtonbox);
	    Box temRandomButtonBox = Box.createHorizontalBox();
	    temRandomButtonBox.add(tenRandomButton);
	    optionBox.add(temRandomButtonBox);
...
//	    picturesToClassifyBox.add(rangeText);
	    Box thoseClickedOnButtonBox = Box.createHorizontalBox();
	    thoseClickedOnButtonBox.add(thoseClickedOnButton);
//	    optionBox.add(thoseClickedOnButtonBox);
...
//	    ClearClassifierButton.setEnabled(false);
	    ...
	    RunClassifyAndPostProcessButton=new JButton("<html>Classify & Centers</html>");
	    ...
	    Box setExclusionButtonBox = Box.createHorizontalBox();
	    setExclusionButtonBox.add(SetExclusionsButton);
//	    optionBox.add(setExclusionButtonBox);
...
//	    buttonBox.add(find_centers_method_panel);	    
	    ...
	    Container find_centers_method_panel_box = Box.createVerticalBox();
...
//	    find_centers_method_panel_box.setLayout(new GridLayout(0, 1, 2, 0));
	    ...
	    segmentation_method_radio_button = new JRadioButton("Segmentation");
	    segmentation_method_radio_button.setSelected(true); //it's the default as of now
	  
	    find_centers_method_panel_box.add(segmentation_method_radio_button);	    
	    ButtonGroup findCentersMethodsGroup = new ButtonGroup();
	    findCentersMethodsGroup.add(erosion_method_radio_button);
	    findCentersMethodsGroup.add(segmentation_method_radio_button);
	    
	    centroidSettingsButton = new JButton("settings");

	    find_centers_method_panel_box.add(centroidSettingsButton);
	...
	private void makeCentroidSettingsBox() {
		centroidSettingsBox = Box.createVerticalBox();
		
		
		for (final String ph: Run.it.getPhenotyeNamesSaveNONAndFindCenters()){
			Box phenotypeBox = Box.createVerticalBox();
			
			phenotypeBox.setBorder(new LineBorder(Color.black));
			if (Run.it.getPhenotypeDistMap()==null)
				Run.it.setPhenotypeDistMap(new HashMap<String, UnivariateDistribution>());
			
			UnivariateDistribution thisDist = Run.it.getPhenotypeDistMap().get(ph);
			if (thisDist == null){
				thisDist = new UnivariateDistribution();
				Run.it.getPhenotypeDistMap().put(ph,thisDist);
			}
			
			if (thisDist.getCutoffBottom() == 0)
				thisDist.setCutoffBottom(Run.DEFAULT_CUTOFF_BOTTOM);
			if (thisDist.getCutoffTop() == 0)
				thisDist.setCutoffTop(Run.DEFAULT_CUTOFF_TOP);
			if (thisDist.getCutoffMid() == 0)
				thisDist.setCutoffMid(Run.DEFAULT_CUTOFF_MID);

			final JSpinner cutoffTopSpinner = new JSpinner(new SpinnerNumberModel(thisDist.getCutoffTop(),1,Integer.MAX_VALUE,1));
			final JSpinner cutoffMidSpinner = new JSpinner(new SpinnerNumberModel(thisDist.getCutoffMid(),1,Integer.MAX_VALUE,1));
			final JSpinner cutoffBottomSpinner = new JSpinner(new SpinnerNumberModel(thisDist.getCutoffBottom(),1,Integer.MAX_VALUE,1));


			cutoffTopSpinner.addChangeListener(
					new ChangeListener(){
						public void stateChanged(ChangeEvent e) {
							Run.it.getPhenotypeDistMap().get(ph).setCutoffTop((Integer) cutoffTopSpinner.getValue());
							Run.it.GUIsetDirty(true);
//							System.out.println((Integer)Run.it.getPhenotypeDistMap().get(ph).getCutoffTop());
						}	
					}
			);


			cutoffMidSpinner.addChangeListener(
					new ChangeListener(){
						public void stateChanged(ChangeEvent e) {
							Run.it.getPhenotypeDistMap().get(ph).setCutoffMid((Integer) cutoffMidSpinner.getValue());
							Run.it.GUIsetDirty(true);
						}	
					}
			);

			cutoffBottomSpinner.addChangeListener(
					new ChangeListener(){
						public void stateChanged(ChangeEvent e) {
							Run.it.getPhenotypeDistMap().get(ph).setCutoffBottom((Integer) cutoffBottomSpinner.getValue());
							Run.it.GUIsetDirty(true);
						}	
					}
			);
			...
			b1.add(new JLabel("cutoffTop:"));
			b1.add(cutoffTopSpinner);

			Box b2 = Box.createHorizontalBox();
			b2.add(new JLabel("cutoffMid:"));
			b2.add(cutoffMidSpinner);

			Box b3 = Box.createHorizontalBox();
			b3.add(new JLabel("cutoffBottom:"));
			b3.add(cutoffBottomSpinner);
			
			Box b0 = Box.createHorizontalBox();
			b0.add(Box.createHorizontalGlue());
			b0.add(new JLabel("Phenotype: " + ph));
			b0.add(Box.createHorizontalGlue());
			...
			centroidSettingsBox.add(phenotypeBox);
		}

	}
	private void showCentroidSettingsFrame(){
		if (settingsFrame != null){
			settingsFrame.setVisible(false);
			settingsFrame.dispose();
			settingsFrame = null;
		}
		settingsFrame = new JFrame("Centroid Finder Settings");
		settingsFrame.add(centroidSettingsBox);
	    settingsFrame.pack();
	...
	private void SetUpListeners(){		
		multipleFramesCheck.addChangeListener(
				new ChangeListener(){
					public void stateChanged(ChangeEvent e){
						if (multipleFramesCheck.isSelected()){
							Run.it.multiple_frames_check=1;
							FilmFrameBeforeAndAfterDatumSetup.setMultipleFrames();
						}
						else{
							Run.it.multiple_frames_check=0;
							FilmFrameBeforeAndAfterDatumSetup.setSingleFrame();
						}
						Run.it.GUIsetDirty(true);
					}
				});
...
//						nRandomText.setEnabled(false);
		...
		centroidSettingsButton.addActionListener(
				new ActionListener(){
					public void actionPerformed(ActionEvent e){
						makeCentroidSettingsBox();
						showCentroidSettingsFrame();
					}
				}
			);
...
//						ReenableStopButton();
		...
		segmentation_method_radio_button.addActionListener(
			new ActionListener(){
				public void actionPerformed(ActionEvent arg0) {
					Run.it.imageset.setCentroidFinderMethod(CentroidFinder.SegmentationLabelMethod);
					Run.it.GUIsetDirty(true);
				}
			}
		);
...
//		rangeText.setEnabled(false);
		...
		numBatchLabel.setEnabled(false);
...
//		System.out.println("KClassifyPanel.AddOrEditInitialProgressBars()");
		remove(centerBox);
		for (Component c:centerBox.getComponents())
			if (c == preProcessingBarsBox)
				centerBox.remove(preProcessingBarsBox);
...
//		System.out.println("before add to center");
...
//		System.out.println("after repaint");
	...
	public void SetValuesToOpenProject(){
	    ...
	    multipleFramesCheck.setSelected((Boolean)(Run.it.multiple_frames_check == 0));
	    multipleFramesCheck.doClick();
//		numPiecesSpinner.setValue((Integer)Run.it.NUM_SPLIT_PIECES);
//		numRotationsSpinner.setValue((Integer)Run.it.NUM_ROTATIONS);
...
//		rangeText.setText(Run.it.RANGE_TEXT);
		switch (Run.it.imageset.getCentroidFinderMethod()){
			case CentroidFinder.LabelViaSmartErosionsMethod:
				erosion_method_radio_button.setSelected(true);
				segmentation_method_radio_button.setSelected(false);
				break;
			default: 
				erosion_method_radio_button.setSelected(false);
				segmentation_method_radio_button.setSelected(true);
		}
	}
