...
public abstract class KTrainPanel extends KPanel implements TrainClickListener,SelectionEmulator{
	...
	/** the user wishes to view all training points */
	protected JButton viewAllTrainingPoints;
		...
		classDownAction.setAction(
			new AbstractAction(){
				public void actionPerformed( ActionEvent e ) {
					...
					for (int i=0;i<list.size();i++)
						if (list.get(i) == activeInfo){
//								System.out.println("it is #"+i+" of "+list.size());
...
//		);
	...
	public KThumbnail AddNewThumbnail(final String filename){
		...
		synchronized (thumbnailPane){
			...
			if (thumbnail != null){
	//			System.out.println("chosen thumbnail filename: " + thumbnail.getFilename());
				final String finalfilename = thumbnail.getFilename();
				DataImage displayImage=null;
				try {
					displayImage = ImageAndScoresBank.getOrAddDataImage(finalfilename);
...
//					System.out.println("text:("+text+")");
	...
	/** spawns dialog to find all training points and manipulate them */
	protected abstract void SpawnViewAllTrainingPointsDialog();
	...
	protected void CreateListeners(){
		...
		viewAllTrainingPoints.addActionListener(
			new ActionListener(){
				public void actionPerformed(ActionEvent e) {
					SpawnViewAllTrainingPointsDialog();					
				}
			}
		);
			...
			new ActionListener(){
				public void actionPerformed(ActionEvent e) {
					...
					try {
						...
						System.err.println("CursorIndexOutOfBoundsError");
					}
					((KImageTrainPanel)imagePanel).setRedrawImage(true);
				}
			}
...
//		System.out.println("inside EnableTrainingHelper Color");
