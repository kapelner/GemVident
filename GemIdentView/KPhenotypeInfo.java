...
public class KPhenotypeInfo extends KClassInfo {
	...
	protected void EditWestBox() {
		if (!((Phenotype)trainer).isNON()||true){
	...
	private void buildExampleImageFromImageSnippets(){
		...
		for (String filename:Run.it.getPhenotypeTrainingImages()){
			DataImage trainingImage=null;
			try {
				trainingImage = ImageAndScoresBank.getOrAddDataImage(filename);
		...
		if (length == 0){
			...
			for (int i=0;i<nrow;i++){
				for (int j=0;j<ncol;j++){
					...
					BufferedImage temp;
					try {
						temp = (ImageAndScoresBank.getOrAddDataImage(ip.filename)).getAsBufferedImage();
...
//		System.out.println("buildExampleImage");
