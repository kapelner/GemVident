...
public class KPhenotypeTrainPanel extends KTrainPanel {
	...
	private JCheckBox confusionLocatorBlinker;
	private JLabel confusionLocatorBlinkerLabel;
	private JLabel confusionLocatorSliderLabel;
	private JSlider confusionLocatorSlider;
...
//					System.out.println("F4");
		...
		forwBoostA.setAction(
			new AbstractAction(){
				public void actionPerformed( ActionEvent e ) {
//					System.out.println("forwBoost keybd");
		...
		backBoostA.setAction(
			new AbstractAction(){
				public void actionPerformed( ActionEvent e ) {
//					System.out.println("backBoost keybd");
	...
	protected void CreateWest() {		
		...
		viewAllTrainingPoints.setText("view points");
		
		TrainHelperButton.addActionListener(
			new ActionListener(){
				public void actionPerformed(ActionEvent e){
					if (Run.it.imageset instanceof ColorVideoSet){
						((ColorVideoSet)Run.it.imageset).CreateFilmReelTrainer(that);
					}					
					else if (!(Run.it.imageset instanceof NuanceImageListInterface))
...
//		System.out.println("add thumb to retrain:"+filename);
		if (Run.it.imageset.PicWasClassified(filename)){
//			System.out.println("pic was classified:"+Classify.GetIntermediateName(filename));
...
//			thumbnail.setFilename(Classify.GetIntermediateName(filename));
		}
		if (add)
	...
	private ViewTrainingPoints viewTrainingPoints;
	protected void SpawnViewAllTrainingPointsDialog(){
		if (viewTrainingPoints != null){
			viewTrainingPoints.setVisible(false);
			viewTrainingPoints.dispose();
		}
		viewTrainingPoints = new ViewTrainingPoints(Run.it.getPhenotypes());
	}
...
//		JLabel centroidLabel=new JLabel("centroid visibility");
		...
		for (Phenotype phenotype : phenotypes){
			phenNum++;
//			System.out.println("KTrainPanel.AddOrEditSliders() - phenotype:"+phenotype.getName());
...
//			centroidblinker.setAlignmentX(Component.RIGHT_ALIGNMENT);
			...
			switch (phenNum){
				case 1: 
//					System.out.println("KTrainPanel.AddOrEditSliders() - switch (phenNum) - case1");
					...
					break;
				case 2:
//					System.out.println("KTrainPanel.AddOrEditSliders() - switch (phenNum) - case2");
					...
					break;
				case 3:
//					System.out.println("KTrainPanel.AddOrEditSliders() - switch (phenNum) - case3");
					...
					break;
				case 4:
//					System.out.println("KTrainPanel.AddOrEditSliders() - switch (phenNum) - case4");
...
//		System.out.println("train pt:" + to.toString());
	...
	private static final String confusionLocatorBlinkerTitle = "Confusion Blinker: ";	
	private static final String confusionLocatorSliderTitle = "Confusion Slider: ";
	private static boolean confusion_not_blinking = true;
	
	/** during retraining, the difficulty for GemIdent overlay feature is enabled */
	public void SetupConfusionLocator(){
		if (confusionLocatorBlinkerLabel == null){ //only do it if it hasn't been done before
			confusionLocatorBlinkerLabel=new JLabel(confusionLocatorBlinkerTitle, JLabel.RIGHT);
			confusionLocatorBlinker = new JCheckBox(); 
			confusionLocatorBlinker.setSelected(false);
			confusionLocatorBlinker.addItemListener(
				new ItemListener(){
					public void itemStateChanged(ItemEvent e) {
					    if (e.getStateChange() == ItemEvent.DESELECTED){
					    	confusion_not_blinking = true;
					    	imagePanel.repaint();
					    	imagePanel.repaintMagnifier();
					    }				    	
					    else {
					    	confusion_not_blinking = false;
					    	MakeBlinkerThread().start();				    	
					    }				    
					}
	
					private Thread MakeBlinkerThread() {
						return new Thread(){
							public void run(){
								while (true){
									try {
									...
									if (confusion_not_blinking){
										return;
									}
									((KImagePhenotypeTrainPanel)imagePanel).setDisplayConfusionImage(false);
									imagePanel.repaint();
									imagePanel.repaintMagnifier();
									try {
										Thread.sleep(BlinkerPeriod);
									...
									((KImagePhenotypeTrainPanel)imagePanel).setDisplayConfusionImage(true);
									imagePanel.repaint();
									imagePanel.repaintMagnifier();
								}
							}
			...
			confusionLocatorSliderLabel=new JLabel(confusionLocatorSliderTitle, JLabel.RIGHT);
			confusionLocatorSlider = new JSlider(JSlider.HORIZONTAL, 0, KImagePhenotypeTrainPanel.MaxConfusionMeasure, 0);
			confusionLocatorSlider.setPreferredSize(new Dimension(80, confusionLocatorSlider.getHeight()));
			confusionLocatorSlider.addChangeListener(
				new ChangeListener(){	
					public void stateChanged(ChangeEvent e) {
						((KImagePhenotypeTrainPanel)imagePanel).setConfusionMeasureAndRefresh(confusionLocatorSlider.getValue());
						((KImagePhenotypeTrainPanel)imagePanel).setDisplayConfusionImage(true);
						imagePanel.repaint();
					}
				}
			);
			//tick marks for confusion slider
//			confusionLocatorSlider.setMajorTickSpacing(10);
			confusionLocatorSlider.setMinorTickSpacing(5);
			confusionLocatorSlider.setPaintTicks(true);
			confusionLocatorSlider.setSnapToTicks(false);
			
			settings_box.setLayout(new GridLayout(0,2,6,0));
			settings_box.add(confusionLocatorBlinkerLabel);
			settings_box.add(confusionLocatorBlinker);	
			settings_box.add(confusionLocatorSliderLabel);
			settings_box.add(confusionLocatorSlider);
		}
	}
	
	public void DisableConfusionLocator(){
		if (confusionLocatorBlinkerLabel == null){
			SetupConfusionLocator();
		}
		confusionLocatorBlinkerLabel.setEnabled(false);
		confusionLocatorBlinker.setEnabled(false);
		confusionLocatorSliderLabel.setEnabled(false);
		confusionLocatorSlider.setEnabled(false);
	}
	
	public void ReenableConfusionLocator(){
		if (confusionLocatorBlinkerLabel == null){
			SetupConfusionLocator();
		}		
		confusionLocatorBlinkerLabel.setEnabled(true);
		confusionLocatorBlinker.setEnabled(true);
		confusionLocatorSliderLabel.setEnabled(true);
		confusionLocatorSlider.setEnabled(true);
	}
