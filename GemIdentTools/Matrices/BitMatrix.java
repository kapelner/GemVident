...
/**
 * <p>
 * A class that stores logical matrices in an efficient 1-bit per value format. A watered down implementation
 * of the BitMatrix class in the COLT CERN libraries. 
 * </p>
 * <p>
 ...
 * @see <a href="http://dsd.lbl.gov/hoschek/colt/">COLT libraries</a>
 */
...
public BitMatrix(int columns, int rows) {
	elements(QuickBitVector.makeBitVector((long)(columns)*(long)rows),columns,rows);
}
/**
...
public BitMatrix(int columns,int rows,long[] bits){
	elements(bits,columns,rows);
}
public long[] getBits(){
	return bits;
}
...
public void xor(BitMatrix other) {
	...
	public BitMatrix crop(int xo, int xf, int yo, int yf) {
		BitMatrix cropped = new BitMatrix(xf - xo, yf - yo);
		for (int i = xo; i < xf; i++){
			for (int j = yo; j < yf; j++){
				cropped.putQuick(i - xo, j - yo, get(i, j));
			}		
		}
		return cropped;
	}
